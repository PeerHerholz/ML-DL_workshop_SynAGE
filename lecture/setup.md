...
Now, the only thing that's still missing is the actual workshop content. Do get this, use again a (docker) terminal and run the following command
    ...
    docker pull peerherholz/ml-dl_workshop_synage
...
For example, something like `docker run -p 8888:8888 -it --rm -v /path/to/your/output_folder:/output peerherholz/ml-dl_workshop_synage`, where `/path/to/your/output_folder` should be an empty folder on your system, such as `/User/neuro/Desktop/output`.
...
Here's a more detailed explanation of this full command:
    ...
    docker run \                    #  start up a container already built or pulled
        ...
        peerherholz/ml-dl_workshop_synage    #  use specified user/project:version container
...
1. Download the [`environment.yml`](https://raw.githubusercontent.com/peerherholz/ML-DL_workshop_SynAGE/master/environment.yml) file (e.g. with right mouse click -> Save As). Make sure that the file ends with `.yml` and not `.txt`.
...
3. Download the notebooks in this repository via [this link](https://github.com/peerherholz/ML-DL_workshop_SynAGE/archive/master.zip)) and unzip them to your prefered location, e.g. `Desktop/ML-DL_workshop_SynAGE`.
4. Next, open up a `conda terminal` (or any other `terminal`), activate the `conda environment` with `conda activate ml-dl_workshop` (or on older `conda environment` with `source activate ml-dl_workshop` for `mac` and `linux` and `activate ml-dl_workshop` for `windows`).
5. Finally, via the `terminal`, move to the folder where you've put all the unzipped content of this workshop, e.g. with the command `cd ~/Desktop/ML-DL_workshop_SynAGE` and run the command `jupyter notebook` from the folder that contains the `ml-dl_workshop_synage_overview.ipynb` notebook.
```
...
You can use this approach to run and test most of the notebooks and to explore the slides. To access the MyBinder instance, use [this link](https://mybinder.org/v2/gh/peerherholz/mybinder_ML-DL_workshop_SynAGE/HEAD).
```
